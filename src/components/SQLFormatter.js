const SQLFormatter = {
  ...
  methods: {
    ...
    convertToList(inputText) {
      const lines = this.inputText.split(/\r?\n/) // split by lines (handles Windows/Mac/Linux)
      this.outputText = lines      
        .map(s => s.trim().toUpperCase())             // trim whitespace
        ...
        .map(s => `'${s.replace(/'/g, "\\'")}'`) // wrap in single quotes, escape internal '
        .join(', ');
    }
  }
